...
function sst_temp_soorathesab_baravorde_khesarat( $gharardad, $rows, $now_date, $khodro_obj ) {
	global $dbprefix;
	if ( $gharardad[ 'gharardad_baravorde_khesarat' ] > 0 ) {
		//dbg('sssssss');
		//dbg($gharardad[ 'gharardad_baravorde_khesarat' ]);
		...
		$variz_bedehi = sst_find_variz_bedehi_jari( $gharardad[ 'gharardad_mostajer_id' ], $gharardad[ 'gharardad_mojer_id' ], $asli_shakhs_id, $shakhs_id, $gharardad[ 'gharardad_baravorde_khesarat' ] );
		$new_temp_soorathesab_baravorde_khesarat = array(
			"aid" => $ashkhas_obj->id,
			...
			"ssoorathesab_babat" => 'خسارت جاری-' . $gharardad[ 'gharardad_sharhe_khesarat' ],
		...
		//dbg($new_temp_soorathesab_baravorde_khesarat);
		//$rows[]=$new_temp_soorathesab_ayab_zahab;
		array_unshift( $rows, $new_temp_soorathesab_baravorde_khesarat );
		//echo '11111111111111';
	}
	return $rows;
