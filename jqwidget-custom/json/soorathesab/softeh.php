...
function sst_temp_soorathesab_softeh( $gharardad, $rows, $now_date, $khodro_obj ) {
	global $dbprefix;
	//dbg($ashkhas_obj);
	if ( $gharardad[ 'gharardad_hazineye_softeh' ] > 0 ) {
		...
		$variz_bedehi = sst_find_variz_bedehi_jari( $gharardad[ 'gharardad_mostajer_id' ], $gharardad[ 'gharardad_mojer_id' ],$asli_shakhs_id,$shakhs_id,$gharardad[ 'gharardad_hazineye_softeh' ]);
		
		$new_temp_soorathesab_softeh = array(
			"aid" => $ashkhas_obj->id,
			...
			"ssoorathesab_babat" => 'هزینه سفته جاری',
		...
		array_unshift( $rows, $new_temp_soorathesab_softeh );
	}
	return $rows;
}