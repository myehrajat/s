...
function sst_temp_soorathesab_ayab_zahab( $gharardad, $rows, $now_date, $khodro_obj ) {
	global $dbprefix;
	//dbg($ashkhas_obj);
	if ( $gharardad[ 'gharardad_hazineye_ayab_zahab' ] > 0 ) {
		...
		$variz_bedehi = sst_find_variz_bedehi_jari( $gharardad[ 'gharardad_mostajer_id' ], $gharardad[ 'gharardad_mojer_id' ],$asli_shakhs_id,$shakhs_id,$gharardad[ 'gharardad_hazineye_ayab_zahab' ]);
		
		$new_temp_soorathesab_ayab_zahab = array(
			"aid" => $ashkhas_obj->id,
			...
			"ssoorathesab_babat" => 'ایاب و ذهاب جاری',
		...
		array_unshift( $rows, $new_temp_soorathesab_ayab_zahab );
	}
	return $rows;
}