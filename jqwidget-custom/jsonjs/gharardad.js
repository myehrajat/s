...
jQuery(document).ready(function ($) {
	...
	if (getValues.nosoorat) {
		jsonparam.nosoorat = getValues.nosoorat;
	}
	if (getValues.nochecked) {
		jsonparam.nochecked = getValues.nochecked;
	}	
	if (getValues.nojari) {
		jsonparam.nojari = getValues.nojari;
	}	
	if (getValues.nobaygani) {
		jsonparam.nobaygani = getValues.nobaygani;
	}	
	if (getValues.tarikh) {
	...
	jQuery.get(url, jsonparam).done(function (data) {
		...
		jQuery.get(tempurl).done(function (data) { //this get is only for reading temp file data which its compressed string is in h js lang variable
			...
			****************************************************************/
			if (getValues.khodro) {
				title = title+' '+localdata[0].kkhodro_khodro;
			}
			if (getValues.shakhs) {

				if(localdata[0].ggharardad_mojer_id==getValues.shakhs){
				title = title+' با '+localdata[0].mojerashkhas_nam+' '+localdata[0].mojerashkhas_name_khanevadegi;
				}
				
				if(localdata[0].ggharardad_mostajer_id==getValues.shakhs){
				title = title+' با '+localdata[0].mostajerashkhas_nam+' '+localdata[0].mostajerashkhas_name_khanevadegi;
				}
			}
			if (getValues.tarikh) {
				...
				},*/
				{
					...
					aggregates: ['avg'],
					 aggregatesrenderer: function (aggregates) {
                      ...
                      var renderstring = '<div style="float: right; margin: 2px; overflow: hidden;">' + Math.round(value) + '</div>';
                      return renderstring;
                  }
				}, {
					text: "ک مجاز روزانه",
			...
			var initrowdetails = function (index, parentElement, gridElement, datarecord) {
				...
				text += "</tr>";
				text += "<tr>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px;  ' >" + return_not_null(datarecord.ggharardad_odo_mojaz) + "</div></td>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px; '>" + return_not_null(datarecord.ggharardad_odo_avaliye) + "</div></td>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px; '>" + return_not_null(datarecord.ggharardad_odo_sanaviye) + "</div></td>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px; '>" + return_not_null(datarecord.ggharardad_odo_hazine) + "</div></td>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px; '>" + return_not_null(datarecord.ggharardad_benzin_avaliye) + "</div></td>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px; '>" + return_not_null(datarecord.ggharardad_benzin_sanaviye) + "</div></td>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px; '>" + return_not_null(datarecord.gharardad_tahvil_dahandeashkhas_nam) + " " + return_not_null(datarecord.gharardad_tahvil_dahandeashkhas_name_khanevadegi) + "</div></td>";
				text += "<td><div class='jqx-grid-cell-right-align' style='margin-top:4px; '>" + return_not_null(datarecord.gharardad_tahvil_girandeashkhas_nam) + " " + return_not_null(datarecord.gharardad_tahvil_girandeashkhas_name_khanevadegi) + "</div></td>";
				text += "</tr>";
				text += "</table>";
