...
function sst_month_term_text_sabt_mah($start,$end,$gharardad_gheymate,$status,$gharardad_mostajer_id,$gharardad_mojer_id,$khodro_id,$save_id){
	$d31=31;
	$d30=30;
	$d29=29;
		...
		if($month<=6 and $remained_days>=31){//31 rooz
			...
			if($status=='اجاره جاری'){
				$jari = sst_jari_rooz(
					...
					$khodro_id,
					($counter).'-'.$status.'-ماهیانه ماه شمار'.'-30روز'.'*'.$gharardad_gheymate,
					$save_id);
				if($jari!=NULL){
					$jari_array[] = $jari;
				}
			}elseif($status=='اجاره'){
				echo ($counter).'-'.$status.'-ماهیانه ماه شمار'.'-30روز='.ceil($gharardad_gheymate*30).'تومان<br>';
				if($calculated_days<$all_days){
					...
					$khodro_id,
					($counter).'-'.$status.'-'.'ماهیانه ماه شمار'.'-'.'30'.'روز'.'*'.$gharardad_gheymate,#$text
					$status,#$status
					$save_id);
				}else{//this is last output 
					register_shutdown_function ('sst_register_long_term_rooz',$insert_rooz);
					return ($counter).'-'.$status.'-ماهیانه ماه شمار'.'-30روز'.'*'.$gharardad_gheymate;
				}
			}
		//برج 12 غیر کبیسه 29 روزه
		}elseif(($month==12 and !sst_is_leap_year(sst_get_year_jalali($tarikh))) and  $remained_days>=29){
			//dbg('29 rooz');
			...
			if(strtolower(sst_get_option( 'mohasebe_doreye_kamel' ) ) == 'yes'){
				if($month<=6){//31 rooz
					...
					if($status=='اجاره جاری'){
						...
						echo ($counter).'-'.$status.'-ماهیانه روزشمار'.'-30روز='.ceil($gharardad_gheymate*30).'تومان<br>';					if($calculated_days<$all_days){
							$insert_rooz[] = array($gharardad_mostajer_id,
							$gharardad_mojer_id,
							$tarikh,#$tarikh
							ceil($gharardad_gheymate*30),#$mablagh
				...
				if($status=='اجاره جاری'){
					$jari = sst_jari_rooz(
						...
						$khodro_id,
						($counter).'-'.$status.'-ماهیانه ماه شمار'.'-'.$remained_days.'روز'.'*'.$gharardad_gheymate,
						$save_id);
					if($jari!=NULL){
						$jari_array[] = $jari;
					}
				}elseif($status=='اجاره'){
					echo ($counter).'-'.$status.'-ماهیانه ماه شمار'.'-'.$remained_days.'روز='.ceil($gharardad_gheymate*$remained_days).'تومان';
					register_shutdown_function ('sst_register_long_term_rooz',$insert_rooz);
					return ($counter).'-'.$status.'-ماهیانه ماه شمار'.'-'.$remained_days.'روز'.'*'.$gharardad_gheymate;
				}
			}
		}
		$counter++;
